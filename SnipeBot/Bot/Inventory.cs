...
namespace SnipeBot.Bot
{
    public class Inventory
    {
        ...
        public async Task<IEnumerable<PokemonData>> GetHighestsPerfect(int limit)
        {
            var myPokemon = await GetPokemons();
            var pokemons = myPokemon.ToList();
            return pokemons.OrderByDescending(PokeAccount.CalculatePokemonPerfection).Take(limit);
        }
        ...
        public async Task<PokemonData> GetLastCaughtPokemon(PokemonData match)
        {
            ...
            return
                inventory.InventoryDelta.InventoryItems.Select(i => i.InventoryItemData?.PokemonData)
                    ...
                    && PokeAccount.CalculatePokemonPerfection(p) == PokeAccount.CalculatePokemonPerfection(match)).First();
        }
